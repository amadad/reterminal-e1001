...
## 2026-04-23 — Ghosting fixed by splitting refresh policy: full on content change, partial on navigation
...
- **Root cause(s):** two compounding issues. (1) The 2026-04-17 partial-only policy was correct for the minute-cadence architecture that existed then but became a liability under the current hourly feed because the firmware never distinguished between "new content landed" (needs clean slate) and "user is paging between existing slots" (already clean). (2) The firmware never called `display.hibernate()` after refresh cycles, so the UC8179 controller left panel driving voltages on between updates, contributing to residual charge — a deviation from the GxEPD2 canonical example.
- **Intermediate attempt and reversal:** a first pass flipped the default to "full refresh on every update" which fixed the ghosting but made every L/R button press flash. That was too blunt — Kindle handles this with a split: full on content change, partial on navigation. Partial refresh between already-loaded slots does not accumulate ghosts because the stored pixels do not change between navigations (ghost accumulation is a function of *pixel churn per refresh*, not *partial refresh count*).
- **Final fix:** `/imageraw` to currently-visible slot, boot restore, and right-button press all do full refresh. `/page` switches and left/middle button navigation do partial refresh. Every path ends with `display.hibernate()`. See CLAUDE.md rule 8.
- **Evidence:** before any firmware change, triggering three `POST /page?full=1` calls on the live device produced the cleanest display in recent memory — refuting the 2026-04-17 claim that full refresh inherently degrades typography. Final firmware flashed via USB (CH340 at `/dev/cu.usbserial-1410`, build `Apr 23 2026 10:05:03`). Slot navigation is now flash-free; content push still lands clean.
- **Gotcha (recurring):** the 2026-04-17 entry below is still instructive — at minute-or-faster cadence, repeated full refreshes accumulate contrast loss. The rule is cadence-aware. If a scene starts pushing the same slot faster than once every few minutes, that specific push path may need its own throttling or a revisit.
- **OTA gotcha:** OTA flash from this Mac fails with "Host Not Found" despite the device advertising `_arduino._tcp`. Diagnosis: macOS Application Firewall drops the UDP reply from the device to espota's ephemeral port. USB flash via `/dev/cu.usbserial-1410` works reliably. To fix OTA, either allow Python through macOS firewall or disable the firewall briefly for the flash.
