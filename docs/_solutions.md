...
## 2026-09-08 — Delivery receipts and complete-day calendar acquisition prepared

- **Observed gaps:** a healthy publisher did not prove device delivery; the external agenda helper exported only events remaining after the current time; expired seasonal cards owned slots indefinitely; navigation restarted the device's polling interval. Source inspection also found partial downloads could overwrite an in-memory last-good bitmap.
- **Changes:** a repo-owned structured calendar exporter retains full days, end times, timezone, status, locations, and an explicit check timestamp. It reuses the 20-minute launchd job and atomically retains the prior source on failure. The revised edition uses Now / Week / optional Quest with Tomorrow fallback / Weekend, readable source timestamps, and explicit seasonal expiry. Host health and doctor distinguish rendering, HTTP downloads, stored receipts, and returned display-driver calls; receipt history persists across restart. Rendering installs whole editions atomically. New firmware verifies full hashes, bounds downloads, stages storage writes, reports pull errors, refreshes on diagnostic entry, and preserves the polling deadline through navigation.
- **Verification:** 198 Python tests passed; Ruff and dependency audit passed with no known vulnerabilities. PlatformIO compiled the firmware; five native checks of the actual bounded reader covered complete transfer, trickle, stall, disconnect, and clock wrap. Independent source review found no remaining blocking findings after fixes. All four current-source previews were inspected.
- **Deployment status:** prepared, **not activated**. The running publisher still returns 404 for `/health`; no USB serial device is connected and diagnostic discovery returned no device. Google acquisition requires a credential-cache write outside this session's writable roots; system LaunchDaemon replacement/restart also needs the operator's Terminal. The ignored `artifacts/local/activate-improvements.sh` performs acquisition, previews, backup, service replacement, host restart, and hash-bound route checks with rollback. Activate the host before flashing; old host routes cannot serve the new firmware's hash-bound requests.
- **Remaining physical gate:** flash, rediscover/probe, verify snapshots and actual screen, then observe three changed editions acknowledged by unattended timer wakes. Receipt `refresh_returned` and `displayed_hash` are driver-return evidence, not controller/optical proof. See `docs/delivery.md` and `docs/calendar-source.md`.

## 2026-09-08 — Everyday calendar edition replaces expired summer slot ownership

- **Symptoms:** the running publisher served an August 31 camp week, an August 24–30 trip still labeled ON TRIP, and a quest that expired August 21. The calendar was current; its writer and the publisher were healthy.
- **Cause:** the active manifest permanently pinned seasonal sources with no ongoing writer. Re-rendering those sources could not make their content current.
- **Change:** added `kitchen-calendar.json` with Now / Week / Next / Weekend, all reading the existing 20-minute calendar projection. The five-minute render tick advances dates and event starts without a file write. Missing dates render as unavailable data; all four views retain the two-hour stale indicator. Calendar selection stays PIL-free; new layouts reuse kitchen tokens.
- **Typography:** operator feedback on cramped text led to 28px body text with 36px leading, 18px labels, and 34px titles for the everyday edition. Now/Weekend use equal columns; Week uses up to four full-width rows. Four calendar goldens were deliberately updated; text-boundary tests reject overlapping or off-panel text.
- **Operations:** confirmed both jobs are system LaunchDaemons under `/Library/LaunchDaemons`, not Helm workloads or the archived per-user LaunchAgents. Publisher KeepAlive is enabled; exporter interval is 1200 seconds and its latest inspected exit code was 0.
- **Deployment (16:17 EDT):** the operator ran the prepared activation script, backed up the former manifest, and restarted the system publisher. The active local manifest now owns Now / Week / Next / Weekend; launchd reports the replacement process running. All four LAN-served bitmaps are 48000 bytes and match both their advertised hashes and freshly rendered previews exactly. The exporter remains scheduled every 1200 seconds; the publisher watches one source plus the manifest and re-renders every 300 seconds. 159 tests pass, Ruff passes, and the dependency audit found no known vulnerabilities. Physical screen delivery still awaits a device wake or diagnostic readback; the device was not reachable during the post-activation scan.
- **Device recovery (16:32 EDT):** later discovery reached the diagnostic API. The device reports a May 12 build (`72eae7c5f3b2-dirty`), four loaded slots, and a 1800-second wake interval. Doctor, capabilities and a read-only probe completed before `publish --push --show-slot 0 --live`. All four uploads succeeded; every `/snapshot` returned 48000 bytes matching both the new preview and current publisher payload byte-for-byte. `/page` confirms slot 0 selected. Local evidence: `artifacts/local/device-upload-verification.json` and `visible-now-readback.png`. Subsequent unattended timer-pull delivery still needs a wake-cycle check; this manual recovery does not prove that path or identify why the earlier fetch failed.
