...
- firmware performs an unconditional `ESP.restart()` after 6 hours of uptime (`PERIODIC_RESTART_MS = 21600000`). This defeats the ESP32 "zombie Wi-Fi" state (stack reports connected, TCP dead) which bypasses every conditional restart path. All slots survive via LittleFS. The threshold is overridable via `RETERMINAL_PERIODIC_RESTART_MS` in `platformio.local.ini`. `/capabilities` reports `periodic_restart_ms` and `last_self_restart_reason` (`periodic` | `wifi_stale` | `none`).
- `WiFi.setAutoReconnect(false)` — manual reconnect only via `maintainWifi()`. Combining auto-reconnect with manual `WiFi.begin()` races the ESP32 Wi-Fi state machine and can produce the zombie state.
