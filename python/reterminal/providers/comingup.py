...
def render_comingup(
    ...
    *,
    ...
    img, draw = new_canvas()
    body_top = draw_kicker(draw, "Coming Up")
    ...
    if not shown and not queue_shown:
        draw.text((MARGIN, HEIGHT // 2), "(nothing coming up)", font=BODY, fill=0)
    ...
    if shown:
        ...
        for i, ev in enumerate(shown):
            ...
            draw.text((date_col_x, baseline - 8), date_str, font=META, fill=0)
    ...
    if queue_shown:
        ...
        draw_rule(draw, qy)
        qy += 8
        draw.text((MARGIN, qy), "UP NEXT", font=KICKER, fill=0)
        qy += 22
        for a in queue_shown:
            label = truncate_text(draw, a.label, BODY, WIDTH - MARGIN * 2 - 20)
            draw.text((MARGIN, qy), "•", font=BODY, fill=0)
            draw.text((MARGIN + 20, qy), label, font=BODY, fill=0)
            qy += 28
